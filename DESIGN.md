...
1. **Cell values are numbers.** A forest cell's `value` is the same `list(estimate =, conf_low =, conf_high =)` as an estimate cell, with `type = "plot"`. The ggplots are built inside `as_gt()` (via `gt::text_transform()` + `ggplot_image()`), never stored in the frame — necessarily so, because the x-scale (limits, intercept, breaks, log vs linear) is a property of the *column*, computed across all of its cells and overridable through the block's `axis` options. A cell frame with a forest column snapshots exactly like one without.
